...
int check(int idx, string &pattern, string &text){
    for(int i = 0; i < pattern.size(); i++){
        if(pattern[i] != text[(idx + i) % text.size()]){
            if(pattern[i] < text[(idx + i) % text.size()]){
                return 1;
            } else {
                return -1;
...
void solve(SuffixArray &algo, string &text){
    string pattern; cin >> pattern;
    int po = 0, ko = algo.p.size(), sr;

    while(po + 1 < ko){
        sr = (po + ko)/2;
        if(check(algo.p[sr], pattern, text) >= 0){
            ko = sr;
        } else {
            po = sr;
        }
    ...
    // cout << ko << "\n";
    // cout << text.size() - pattern.size() << " " << algo.p[ko] << "\n";
    // cout << text.substr(algo.p[ko], pattern.size()) << "\n";
    if(ko == algo.p.size() || 
    text.size() - pattern.size() < algo.p[ko] || 
    text.substr(algo.p[ko], pattern.size()) != pattern){
        cout << "0\n";
        return;
    }
    int ans1 = ko;
...
int main(){
    ...
    // for(int i = 0; i < algo.p.size(); i++){
    //     cout << algo.p[i] << " " << text.substr(algo.p[i], text.size() - algo.p[i]) << "\n";
    // }
    // cout << "\n";
    int q; cin >> q;
    while(q--){
